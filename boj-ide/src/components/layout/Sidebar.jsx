...
import { LayoutDashboard, BookOpen, Users, Trophy, SlidersHorizontal, Bell, PanelLeftClose, ChevronsUpDown } from 'lucide-react';
...
export const Sidebar = () => {
    ...
    if (!isOpen)
        return null;
    return (<div className="w-64 h-full bg-[#0F1117] border-r border-slate-800 flex flex-col flex-shrink-0 transition-all duration-300">
      ...
      <div className="h-14 flex items-center justify-between px-3 hover:bg-slate-800/30 transition-colors cursor-pointer group mb-2">
        ...
        <button onClick={() => setIsOpen(false)} className="p-1 text-slate-400 hover:text-slate-100 hover:bg-slate-700/50 rounded opacity-0 group-hover:opacity-100 transition-opacity">
      ...
      <div className="flex-1 px-2 space-y-0.5 overflow-y-auto">
        {menuItems.map((item) => (<button key={item.id} onClick={() => setCurrentPage(item.id)} className={cn('w-full flex items-center gap-2.5 px-3 py-1.5 rounded-md text-sm font-medium transition-colors group', currentPage === item.id
      ...
      <div className="p-2 border-t border-slate-800/50 space-y-0.5">
        ...
        <button onClick={() => setCurrentPage('profile')} className="w-full flex items-center gap-2.5 px-3 py-1.5 rounded-md text-sm font-medium text-slate-300 hover:bg-slate-800/50 transition-colors">
        ...
        {/* Basic Settings */}
        <button onClick={() => setCurrentPage('settings')} className="w-full flex items-center gap-2.5 px-3 py-1.5 rounded-md text-sm font-medium text-slate-400 hover:text-slate-100 hover:bg-slate-800/50 transition-colors">
          <SlidersHorizontal className="w-4 h-4"/>
          <span className="flex-1 text-left">기본 설정</span>
        </button>

        {/* Notifications */}
        <button onClick={() => setCurrentPage('settings')} // Can route to specific tab if needed
