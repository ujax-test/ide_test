...
import { Play, RotateCcw, Settings, CheckCircle2, AlertCircle, Loader2, ArrowLeft, Timer, ChevronDown, ChevronRight, Plus } from 'lucide-react';
...
const Stopwatch = () => {
    ...
    return (<div className="flex items-center gap-2 bg-slate-800 rounded px-2 py-1 text-xs font-mono text-slate-300">
    ...
    return (<div className="flex h-full flex-col bg-[#0F1117] text-slate-300">
      ...
      <div className="h-14 border-b border-slate-800 flex items-center justify-between px-4 bg-[#0F1117]">
        ...
        <div className="flex items-center gap-3">
          ...
          <select value={language} onChange={handleLanguageChange} className="bg-slate-900 border border-slate-700 rounded px-2 py-1 text-xs font-medium text-slate-300 focus:outline-none focus:border-emerald-500 cursor-pointer">
      ...
      <div className="flex-1 flex overflow-hidden">
        ...
        <div className="w-[40%] border-r border-slate-800 flex flex-col bg-[#0F1117] overflow-y-auto custom-scrollbar">
          ...
          <div className="border-b border-slate-800">
            <button onClick={() => toggleSection('problem')} className="w-full flex items-center justify-between px-4 py-3 bg-[#141820] hover:bg-slate-800 transition-colors">
            ...
            {openSections.problem && (<div className="p-6 space-y-6 animate-in slide-in-from-top-2 duration-200">
                <section>
          ...
          <div className="border-b border-slate-800">
            <button onClick={() => toggleSection('tests')} className="w-full flex items-center justify-between px-4 py-3 bg-[#141820] hover:bg-slate-800 transition-colors">
            ...
            {openSections.tests && (<div className="p-6 min-h-[200px] animate-in slide-in-from-top-2 duration-200">
                  {isExecuting ? (<div className="flex flex-col items-center justify-center h-48 text-slate-500">
                      <Loader2 className="w-8 h-8 animate-spin mb-4 text-emerald-500"/>
                      <p>코드를 실행하고 있습니다...</p>
                    </div>) : output ? (<div className="space-y-4">
                    ...
                    </div>) : (<div className="flex flex-col items-center justify-center h-48 text-slate-600">
        ...
        <div className="flex-1 flex flex-col bg-[#1e1e1e]">
          ...
          <div className="h-14 border-t border-slate-800 bg-[#0F1117] px-4 flex items-center justify-between">
            ...
            <div className="flex items-center gap-2">
               <Button variant="primary" size="sm" onClick={executeCode} disabled={isExecuting} className="w-24">
                {isExecuting ? <Loader2 className="w-4 h-4 animate-spin"/> : 'Run Code'}
              </Button>
              <Button className="bg-emerald-600 hover:bg-emerald-700 text-white w-24" size="sm" onClick={handleSubmit}>
                Submit
              </Button>
            </div>
          </div>
