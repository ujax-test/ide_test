...
import { Search, FolderPlus, Folder, ArrowLeft, Plus, MoreVertical } from 'lucide-react';
    ...
    if (!currentBox) {
        ...
        <div className="max-w-7xl mx-auto space-y-8">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {boxes.map((box) => (<div key={box.id} onClick={() => setCurrentBox(box)} className="group bg-slate-900/50 border border-slate-800 rounded-xl p-6 cursor-pointer hover:bg-slate-800/50 hover:border-slate-700 transition-all">
            ...
            <button onClick={() => setIsModalOpen(true)} className="border border-dashed border-slate-800 rounded-xl p-6 flex flex-col items-center justify-center gap-3 text-slate-500 hover:text-emerald-500 hover:border-emerald-500/50 hover:bg-emerald-500/5 transition-all min-h-[160px]">
        ...
        <Modal isOpen={isModalOpen} onClose={() => setIsModalOpen(false)} title="새 문제집 만들기">
          <form onSubmit={handleCreateBox} className="space-y-4">
            <div className="space-y-1">
              ...
              <input type="text" required value={newBox.title} onChange={(e) => setNewBox({ ...newBox, title: e.target.value })} placeholder="예: 코딩테스트 대비 100제" className="w-full bg-slate-900 border border-slate-800 rounded-lg px-4 py-2.5 text-slate-200 focus:outline-none focus:border-emerald-500"/>
            </div>
            <div className="space-y-1">
              <label className="text-sm font-medium text-slate-400">분류</label>
              <select value={newBox.category} onChange={(e) => setNewBox({ ...newBox, category: e.target.value })} className="w-full bg-slate-900 border border-slate-800 rounded-lg px-4 py-2.5 text-slate-200 focus:outline-none focus:border-emerald-500">
        ...
        <div className="flex flex-col gap-6">
          ...
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-4">
              <button onClick={() => setCurrentBox(null)} className="p-2 -ml-2 text-slate-400 hover:text-slate-100 rounded-lg hover:bg-slate-800 transition-colors">
          ...
          <div className="flex gap-4">
            <div className="flex-1 relative">
              ...
              <input type="text" placeholder="문제 검색..." className="w-full h-12 bg-slate-900 border border-slate-800 rounded-lg pl-10 pr-4 text-slate-200 placeholder:text-slate-500 focus:outline-none focus:border-emerald-500 transition-colors"/>
            </div>
          </div>
          ...
          <div className="bg-slate-900/50 border border-slate-800 rounded-xl overflow-hidden">
            ...
            <div className="divide-y divide-slate-800/50">
              {problems.map((problem) => (<div key={problem.id} onClick={() => setPage('ide')} className="grid grid-cols-[80px_1fr_120px_200px_100px] gap-4 p-4 items-center hover:bg-slate-800/30 transition-colors cursor-pointer group">
                  ...
                  <div className="flex gap-2 flex-wrap">
                    {problem.tags.map(tag => (<span key={tag} className="px-2 py-0.5 rounded bg-slate-800 text-slate-400 text-xs border border-slate-700">
                        {tag}
                      </span>))}
                  </div>
                  <div className="text-center">
                    {problem.solved ? (<span className="text-emerald-500 text-xs font-bold">Solved</span>) : (<span className="text-slate-600 text-xs">-</span>)}
                  </div>
                </div>))}
            </div>
          </div>
