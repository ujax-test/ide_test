...
import { Search, Filter, FolderPlus, Folder, ArrowLeft, Plus, MoreVertical } from 'lucide-react';
...
export const ProblemList = () => {
  ...
  const [boxes, setBoxes] = useState([
    { id: 1, title: '코딩테스트 기초 100제', count: 15, color: 'bg-emerald-500' },
    { id: 2, title: '삼성 SW 역량 테스트 기출', count: 8, color: 'bg-blue-500' },
    { id: 3, title: '카카오 블라인드 2024', count: 5, color: 'bg-yellow-500' },
  ]);
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {boxes.map((box) => (
              ...
              >
                ...
                <p className="text-sm text-slate-500">{box.count} 문제</p>
              </div>
            ))}
            
            {/* Create New Box Card (Alternative) */}
            <button className="border border-dashed border-slate-800 rounded-xl p-6 flex flex-col items-center justify-center gap-3 text-slate-500 hover:text-emerald-500 hover:border-emerald-500/50 hover:bg-emerald-500/5 transition-all min-h-[160px]">
