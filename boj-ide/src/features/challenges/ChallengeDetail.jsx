...
import { ArrowLeft, Calendar, CheckCircle2, XCircle, Flame, Trophy, Code2, Medal } from 'lucide-react';
        ...
        <div className="flex flex-col gap-6">
          ...
          <div className="flex items-end justify-between">
            <div>
              <div className="flex items-center gap-3 mb-2">
                 {isEndedView ? (<Badge variant="secondary">Ended</Badge>) : (<div className="flex items-center gap-2">
            ...
            <div className="text-right">
              {isEndedView ? (<div className="flex flex-col items-end">
          ...
          <div className="bg-slate-900/50 border border-slate-800 rounded-xl p-6">
            ...
            <div className="w-full bg-slate-800 rounded-full h-3 overflow-hidden">
              <div className={`h-full bg-gradient-to-r from-emerald-600 to-emerald-400 relative ${isEndedView ? 'w-[92%]' : 'w-2/5'}`}>
                ...
                <div className="bg-slate-900/50 border border-slate-800 rounded-xl overflow-hidden">
                   <table className="w-full text-left border-collapse">
                     ...
                     <tbody className="divide-y divide-slate-800/50">
                       {ranking.map((user, idx) => (<tr key={user.id} className="hover:bg-slate-800/30 transition-colors">
          ...
          <div className="space-y-6">
            ...
            <div className="bg-slate-900/50 border border-slate-800 rounded-xl overflow-hidden">
              ...
              <div className="divide-y divide-slate-800/50 max-h-[400px] overflow-y-auto">
                {participants.map((p) => (<div key={p.id} className="p-3 flex items-center justify-between hover:bg-slate-800/30 transition-colors">
                    <div className="flex items-center gap-3">
                      <div className="relative">
                        <div className="w-8 h-8 rounded-full bg-slate-700 overflow-hidden">
                          <img src={`https://api.dicebear.com/7.x/avataaars/svg?seed=${p.id}`} alt={p.name}/>
                        </div>
                        {!isEndedView && p.status === 'dropped' && (<div className="absolute inset-0 bg-black/60 rounded-full flex items-center justify-center">
                    ...
                    {isEndedView ? (<span className="text-sm font-mono text-emerald-500">{p.score}</span>) : (p.status === 'survived' ? (<div className="flex items-center gap-1 text-orange-400 font-mono text-sm">
                            <Flame className="w-3 h-3 fill-current"/> {p.streak}
                        </div>) : (<Badge variant="destructive" className="text-[10px] py-0 h-5">Failed</Badge>))}
                  </div>))}
              </div>
            </div>
          </div>
